...
public class LinkedList <T extends Comparable<T> >
	...
	public T kthToLastElement(int index) {
		if (index > size || index < 0) {
			throw new IllegalArgumentException("The index cannot be out of bounds on the list.");
		}
		
		int firstRunner = 0;
		int secondRunner = index;
		
		while (secondRunner != size) {
			firstRunner++;
			secondRunner++;
		}

		return getNode(firstRunner).element;
	}
